...
* __병합 정렬의 시간 복잡도__  
분할 단계에서 분할이 되는 깊이는 2의 제곱으로 증가하므로 데이터의 갯수가 n일 때, 깊이는 log_2 n이라 할 수 있다.  
병합 단계에서 단계(깊이) 마다 노드의 갯수는 2^i개, 리스트의 길이는 n/2^i개이다. 노드 마다 리스트의 길이 만큼 비교가 진행되므로 각 단계에서의 시간 복잡도는 <font>$$2^i * \frac { n }{ 2^i } = O(n)$$</font>이다.  
각 단계 마다 O(n)의 시간 복잡도에 단계가 총 log_2 n이므로 O(n) * O(log n) = O(n log n)이라 할 수 있다.  
