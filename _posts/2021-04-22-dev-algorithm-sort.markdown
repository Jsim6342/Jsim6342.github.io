...
* __선택 정렬의 시간복잡도__  
반복문이 2개인 형태로 O($n^2$)으로 볼 수 있다.  
상세하게 계산하면, <font size=5em>$\frac { n * (n - 1)}{ 2 }$</font>  
...
병합 단계에서 단계(깊이) 마다 노드의 갯수는 $2^i$개, 리스트의 길이는 n/$2^i$개이다. 노드 마다 리스트의 길이 만큼 비교가 진행되므로 각 단계에서의 시간 복잡도는 <font size=4em>$2^i * \frac { n }{ 2^i } = O(n)$</font>이다.  
